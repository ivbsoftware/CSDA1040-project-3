...
tail(nasdaqTraded)
```
Now lets take 5 random stocks andrequest their montholy quotes for the last year
```{r fig20, fig.cap="Random stocks Close prices", fig.height=9, fig.width=6, message=FALSE, warning=FALSE, paged.print=FALSE}
set.seed(21)
library("tseries")

stocks <- nasdaqTraded[sample(1:nrow(nasdaqTraded), 5),]
z <- list()
par(mfrow = c(5, 1))
...
```{r fig.height=3, fig.width=5.5, message=FALSE, warning=FALSE, fig.cap="Prediction of stock prices"}
library(forecast)
n <- 4
freq <- 12
toForecast <- 12
x <- ts(z[[n]], frequency=freq)
dmin <- start(z[[n]])
...
name <- toString(stocks[n, "Symbol"][1])
forecasts <- lapply(models, forecast, toForecast)
len <- ceiling(1 + length(x)/freq)
for (f in forecasts) {
  ticks<-format(seq(as.Date(dmin), by="years", length.out = len), format = "%b-%Y")
  plot(f, xlab="", xaxt="n")
